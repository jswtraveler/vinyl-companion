...
- **Test suite** — 162 tests across 7 files covering Album model, genreUtils, diversityFilter, albumNormalization, Scorer, basicRecommendations, useMetadataSearch (Apr 5)
...
### 3. Testing ✅ COMPLETE (Apr 5)

162 tests across 7 files, all passing. Test files live in `src/test/`.

| File | Tests | Covers |
|---|---|---|
| `Album.test.js` | 26 | `validateAlbum`, `createNewAlbum` |
| `genreUtils.test.js` | 32 | `capitalizeGenre`, `extractGenres`, `countAlbumsByGenre`, `calculateGenreOverlap`, `getDistinctGenres` |
| `diversityFilter.test.js` | 15 | `applyDiversityFilter` (MMR + hard constraints + name fallback), `getDiversityStats` |
| `albumNormalization.test.js` | 36 | `AlbumNormalizer`: `normalizeString`, `createFingerprint`, `normalizeTags`, `calculateTagSimilarity`, `extractEraInfo`, `areAlbumsEquivalent` |
| `Scorer.test.js` | 26 | `RecommendationScoring`: constructor, `calculateEraFit`, `calculateTagSimilarity`, `calculateArtistProximity`, `scoreCandidate`, `scoreMultipleCandidates` |
| `basicRecommendations.test.js` | 15 | `mergeMetadataIntoArtists`, `buildArtistRecommendations` |
| `useMetadataSearch.test.js` | 12 | debounce, mode switching, `onApply` callback, skip |

**Not tested (intentional):** database providers, `useAlbumCollection` (integration-level), `GraphRecommender` (PPR disabled — add tests when enabled).

Old broken stubs in `tests/unit/` were deleted.
